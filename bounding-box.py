        ...
        for j in range (-10, 11):
            ...
            if(0 <= newX < height and 0 <= newY < width):
                if(gray_image[newX, newY] > threshold and vis[newX, newY] == 0):
                    ...
                    newBox = dfs(newX, newY, connectedCmp)
...
def makeNNdata(box, connectedCmp):
    ...
    for y in range(width):
        if(vis[x, y] == 0):
            if(gray_image[x, y] > threshold):
                ...
                box = dfs(x, y, connectedComp)
                if(box[4] >= 200 and box[4] <= 5000):
                    data = np.append(data, makeNNdata(box, connectedComp), axis = 0)
